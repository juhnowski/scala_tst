...
trait AbstractTransformer {
  ...
  def onlySyms[A](xs: List[Sym[A]]): List[Sym[A]] = xs map (e => apply(e)) collect { case e: Sym[_] => e.asInstanceOf[Sym[A]] }
...
trait Transforming extends Expressions with Blocks with OverloadHack {
  ...
  // mirroring is (to some extend is just a substitution)
  def mirror[A:Manifest](e: Def[A], f: Transformer): Exp[A] = mirrorDef(e,f)

  def mirrorDef[A:Manifest](e: Def[A], f: Transformer): Def[A] = sys.error("don't know how to mirror " + e)
...
trait FatTransforming extends Transforming with FatExpressions {
  ...
  override def mirror[A:Manifest](e: Def[A], f: Transformer): Exp[A] = (e match {
    case Forward(x) => toAtom(Forward(f(x)))(mtype(manifest[A]))
