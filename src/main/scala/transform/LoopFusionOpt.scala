...
trait LoopFusionOpt extends internal.FatBlockTraversal with LoopFusionCore {
  ...
  override def focusExactScopes[A](resultB: List[Block[Any]])(body: List[Stm] => A): A = {
    val result0 = resultB.map(getBlockResult) flatMap { case Combine(xs) => xs case x => List(x) }
    ...
    if (result0 != result) {
      printlog("\nsuper.focusExactScopeFat with result changed from " + result0 + " to " + result)
    ...
    super.focusExactScopes(result0.map(Block(_)))(body)
  }
...
trait LoopFusionCore extends internal.FatScheduling with CodeMotion with SimplifyTransform {
  val IR: LoopsFatExp with IfThenElseFatExp
  import IR._
  type WTable = List[(Sym[Any], Sym[Any])]
  def experimentalDCE: Boolean = false
...
*/
  ...
  def canFuseIndirect(WtableNeg: WTable, a: Stm, b: Stm): Boolean = 
    !WtableNeg.exists(p => (a.lhs contains p._1) && (b.lhs contains p._2) || (b.lhs contains p._1) && (a.lhs contains p._2))
  def canFuse(WtableNeg: WTable, a: Stm, b: Stm): Boolean = 
    canFuseDirect(a,b) && canFuseIndirect(WtableNeg, a,b)
  ...
  def isShapeDep(s: Exp[Int], a: Stm): Boolean = 
    s match { case Def(SimpleDomain(a1)) => a.lhs contains a1 case _ => false }
  def getShapeCond(s: Exp[Int], a: Stm): List[Exp[Boolean]] = 
    s match { case Def(SimpleDomain(a1)) => WgetLoopRes(a)(a.lhs indexOf a1) match { case SimpleCollectIf(a,c) => c } }


  def extendLoopWithCondition(e: Stm, shape: Exp[Int], targetVar: Sym[Int], c: List[Exp[Boolean]]): List[Exp[Any]] = {
    printlog("\nextending with conds: " + c)
    e.rhs match {
      case SimpleFatLoop(s,x,rhs) => 
        (e.lhs zip rhs).map { case (l,r) => findOrCreateDefinitionExp(SimpleLoop(shape,targetVar,applyAddCondition(r,c)))(mtype(l.tp)) }
    }
  }

  def select[A](a: List[A], b: List[Boolean]): List[A] = (a zip b) collect { case (w, true) => w }
    ...
    */
    ...
    if (Wloops.nonEmpty) {
      ...
      //do{
        ...
        // why only collectIf??
        val loopCollectSyms: List[Sym[Any]] = Wloops flatMap (e => (e.lhs zip WgetLoopRes(e)) collect { case (s, SimpleCollectIf(_,_)) => s })

        // symbols mapped to loops
        val loopSyms: List[Sym[Any]] = Wloops flatMap (_.lhs)
        // vars used in loops
        val loopVars: List[Sym[Int]] = Wloops flatMap WgetLoopVar

        // "direct symbols"
        val WloopSyms: List[List[Sym[Any]]] = Wloops map (_.lhs)
        // "direct vars"
        val WloopVars: List[List[Sym[Int]]] = Wloops map WgetLoopVar
      ...
      //println("Wloops " + Wloops)
        ...
        var WtableNeg: WTable = Wloops.flatMap { dx => // find non-simple dependencies (other than a(i))
          ...
          getSchedule(currentScope)(WgetLoopRes(dx)) flatMap {
            ...
            case sc =>
              ...
              if (pr.nonEmpty) printlog("\nfusion of "+pr+" prevented by " + sc + " which is required by body of " + dx.lhs)// + " / " + thisLoopVars)
              pr
          }
        }.distinct      
        ...
        printlog("\nwtableneg: " + WtableNeg) // will add more later, need to maintain closure
        ...
        for (b@ TTP(_,_,_) <- partitionsIn) {
          ...
          partitionsOut.find(a => canFuse(WtableNeg, a,b)) match {
            case Some(a: TTP) =>
              ...
              printlog("shapeA " + shapeA)
              printlog("shapeB " + shapeB)
              ...
              // it seems that shapes are different iff
              val shape = if (isShapeDep(shapeA,b)) {
                printlog("\nshape dep: shape a depends on b ")
                val conds = getShapeCond(shapeA,b)
                ...
                printlog("\nshape dep: shape B depends on a ")
              ...
              // HA-HA: this fusion is extremely simple!!
        ...
        if ((partitionsOut intersect partitionsIn) != partitionsOut) { // was there any change?
          ...
          printlog("pInT\n" + pInT.mkString("\n"))
          printlog("pOutT\n" + pOutT.mkString("\n"))
          printlog("currentScope:\n" + currentScope.map(_.lhs).mkString(", "))
