...
*/  
  ...
  def transformOne[A](s: Sym[A], x: Def[A], t: SubstTransformer): Exp[A] = {
        ...
        s2 match { 
          case Def(x2) => 
            if (x.getClass == x2.getClass) {
              ...
              val ss2 = syms(x2)
              if (ss2 != tss.filter(_.isInstanceOf[Sym[_]])) // should do filtering in def of tss above?
    ...
    if (y != s) {
      if (y.isInstanceOf[Sym[_]] && findDefinition(y.asInstanceOf[Sym[Any]]).nonEmpty)
  ...
  def transformAll(scope: List[Stm], t: SubstTransformer): List[Stm] = scope flatMap {
    case TP(sym, rhs) =>
      transformOne(sym, rhs, t) match {
        case s: Sym[_] => (scope.find(_.lhs contains s) orElse findDefinition(s)).toList // check scope before target graph
        case _ => Nil
      }
