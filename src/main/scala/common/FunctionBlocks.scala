...
import internal.{Blocks, AbstractTransformer}
import transform._
 ...
 */
trait FunctionBlocksExp extends BaseExp with Blocks with OverloadHack {
  /*
   * BlockN definitions
   */
  implicit def lambdaToBlock0[R:Manifest](f: () => Exp[R]) = Block0(reifyBlock(f()))
  case class Block0[R:Manifest](blockRes: Block[R])
     
  implicit def lambdaToBlock1[T1:Manifest,R:Manifest](f: Exp[T1] => Exp[R]) = {
    val x1 = fresh[T1]
    Block1(x1,reifyBlock(f(x1)))
  }    
  ...
  implicit def lambdaToBlock2[T1:Manifest,T2:Manifest,R:Manifest](f: (Exp[T1],Exp[T2]) => Exp[R]) = {
    ...
    Block2(x1,x2,reifyBlock(f(x1,x2)))
  }
  ...
  override def boundSyms(e: Any): List[Sym[Any]] = e match {
    case Block0(blk) => Nil
    case Block1(b1,blk) => scala.List(b1)
    case Block2(b1,b2,blk) => scala.List(b1,b2)
